<div class="col-sm-5 col-md-3 pr-0">
  <div class="card-list-group card bg-light mb-3">
    ...
    <div class="card-footer p-2 bg-white text-center">
      <a href="#<?php echo isset($post->post_image) && $post->post_image !== '' ? '' : 'imageUploader' ?>" <?php echo isset($post->post_image) && $post->post_image !== '' ? 'data-pid="' . $post->id . '"' . 'id="deletePostImage"' : '' ?> class="smooth-scroll">
        <?php echo isset($post->post_image) && $post->post_image !== '' ? 'Delete' : 'Add' ?> image
      </a>
    </div>
  </div>
</div>